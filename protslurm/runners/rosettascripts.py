...
class RosettaScripts(Runner):
    ...
    def run(self, poses:protslurm.poses.Poses, output_dir:str, prefix:str, xml_path:str, nstruct:int=1, params_path:str=None, options:str=None, pose_options:list or str=None, overwrite:bool=False, jobstarter:protslurm.jobstarters.JobStarter=None) -> RunnerOutput:
        '''Runs rosettascripts.py on acluster'''
        ...
        elif os.path.isfile(rosettascore_path := os.path.join(work_dir, 'rosettascripts_scores.sc')):
        ...
        for pose, pose_opts in zip(poses.df['poses'].to_list(), pose_options):
            for i in range(1, nstruct+1):
                cmds.append(self.write_cmd(pose, output_dir=work_dir, xml_path=xml_path, i=i, rosettascore_path=rosettascore_path, options=options, pose_options=pose_opts))
    ...
    def create_pose_options(self, df:pd.DataFrame, pose_options:list or str=None, fixed_res_column:str=None, design_res_column:str=None) -> list:
    ...
    def write_cmd(self, pose_path:str, output_dir:str, xml_path:str, i:int, rosettascore_path:str, options:str=None, pose_options:str=None):
        ...
        return f"{self.script_path} -parser:protocol {xml_path} -out:path:all {output_dir} -in:file:s {pose_path} -out:prefix r{str(i).zfill(4)}_ -out:file:scorefile {rosettascore_path} {opts} {flags}"
