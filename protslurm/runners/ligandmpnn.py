...
class LigandMPNN(Runner):
    ...
    def __init__(self, script_path:str=protslurm.config.LIGANDMPNN_SCRIPT_PATH, python_path:str=protslurm.config.LIGANDMPNN_PYTHON_PATH, sbatch_options:str=None) -> None:
    ...
    def run(self, poses, jobstarter:protslurm.jobstarters.JobStarter, output_dir:str, model:str="protein_mpnn", options:str=None, pose_options:str=None, return_seq_threaded_pdbs_as_pose:bool=False, preserve_original_output:bool=True) -> RunnerOutput:
        ...
        input_poses = poses.df['poses'].to_list()
        ...
        if os.path.isfile(scorefilepath := os.path.join(work_dir, scorefile)):
            return RunnerOutput(pd.read_json(scorefilepath))

        # parse_options and pose_options:
        pose_options = self.safecheck_pose_options(pose_options, input_poses)

        # write protein generator cmds:
        cmds = [self.write_cmd(pose, output_dir=work_dir, model=model, options=options, pose_options=pose_opts) for pose, pose_opts in zip(input_poses, pose_options)]
    ...
    def collect_scores(self, work_dir:str, scorefile:str, prefix:str, return_seq_threaded_pdbs_as_pose:bool, preserve_original_output:bool=True) -> pd.DataFrame:
        ...
        #scores.columns = [f"{prefix}_{col}" if col not in ['description', 'location'] else col for col in scores.columns]
